...
void schedule(int channel)
{
	...
	if ((CYCLE_VAL % (8*T_REFI)) == 0) {
	  ...
	  for (i=0;i<NUM_RANKS;i++) {
		if (pwrdn[channel][i]) {
		  timedn[channel][i] = timedn[channel][i] + CYCLE_VAL - pwrdn[channel][i];
		  pwrdn[channel][i]=0;
		}
	  }
	}
			...
			if(wr_ptr->command_issuable)
			{
				...
				/* If the command was successful, mark that the rank has now been woken up.  Just book-keeping being done. */
				  if (pwrdn[wr_ptr->dram_addr.channel][wr_ptr->dram_addr.rank]) {
				    timedn[wr_ptr->dram_addr.channel][wr_ptr->dram_addr.rank] = timedn[wr_ptr->dram_addr.channel][wr_ptr->dram_addr.rank] + CYCLE_VAL - pwrdn[wr_ptr->dram_addr.channel][wr_ptr->dram_addr.rank];
			...
			if(rd_ptr->command_issuable)
			{
				...
				/* If the command was successful, mark that the rank has now been woken up.  Just book-keeping being done. */
				  if (pwrdn[rd_ptr->dram_addr.channel][rd_ptr->dram_addr.rank]) {
				    timedn[rd_ptr->dram_addr.channel][rd_ptr->dram_addr.rank] = timedn[rd_ptr->dram_addr.channel][rd_ptr->dram_addr.rank] + CYCLE_VAL - pwrdn[rd_ptr->dram_addr.channel][rd_ptr->dram_addr.rank];
